...
class MDP1(MDP):
    ...
    def __init__(self, init,actlist, terminals, transitions, reward = None, gamma=.9, action_based_rewards = None):
        MDP.__init__(self, init, actlist, terminals, transitions, reward, gamma=gamma)
        self.action_based_rewards = action_based_rewards 
    ...
    def R(self, state,action,next_state):
        """Return a numeric reward for this state.""" 
        return self.action_based_rewards[state][action][next_state] 
...
class MDP2(MDP):
    ...
    def q_value(self, s, a, U):
        ...
        res = 0
        try: 
            for( p, s_prime),(_,r) in zip(self.T(s, a), self.R(s,a)):
                res+=  p * (r + self.gamma * U[s_prime]) 
        except: 
            res += p * (self.R(s) + self.gamma * U[s_prime])
        return res
    def value_iteration(self, epsilon=0.001):
        ...
        while True:
            ...
            for s in self.states: 
                U1[s] = max(self.q_value( s, a, U) for a in self.actions(s))
    ...
    def best_policy(self ):
        ...
        for s in self.states:
            pi[s] = max(self.actions(s), key=lambda a: self.q_value(  s, a, U))
        return pi
