...

{
    ...
    # Handling send events for CBR packets
    if (event == "s" && pn == "cbr" && level == "AGT") {
    ...
    # Handling receive events for CBR packets
    if (event == "r" && pn == "cbr" && level == "AGT") {
    ...
    # Handling dropped packets for CBR packets
    if (event == "d" && pn == "cbr" && pkt_size >= 512) {
        droppedPackets++;       # Increment dropped packet counter
    }

    # Update statistics after every 20 seconds
