...
int check_solutions(struct problem_struct** my_problems, char* solution_file, char* infeas_sln_info_file)
{
    FILE * pfile= fopen(solution_file, "r");
    FILE * pfile2 = fopen(infeas_sln_info_file, "w");
    if(!pfile2){
        printf("Fail to create file %s, please check!\n", infeas_sln_info_file);
        exit(1);
    }
    ...
    if(pfile==NULL)
    {
        ...
        fprintf(pfile2, "Solution file %s does not exist. Please check!\n", solution_file);
        exit(2);
    }
    ...
    if(val != num_of_problems)
    {
        ...
        fprintf(pfile2, "The stated number of solutions does not match the number of problems.\n");
        exit(3);
    }
    struct solution_struct temp_sln;
    int count=0, k=0, count_infeas=0;
    int n, dim;
        ...
        while(fscanf (pfile, "%i", &val)!=EOF)
        {
            ...
            if(count==n)
            {
                ...
                if(!temp_sln.feasibility || fabs(temp_sln.objective - val_obj)>0.01)
                {
                    ...
                    count_infeas++;
                    fprintf(pfile2, "Infeasible solution for instance[%i]\n", k);
                }
                else{
                    break;
                }
