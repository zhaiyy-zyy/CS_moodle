...
struct solution* simulated_annealing_move(struct solution* curt_sln, double temperature) {
    int item1, item2;
            ...
            for (int j = 0; j < NUM_OF_ITEMS; j++) // Iterate over all items again for potential swaps
            {
                ...
                if (i != j && curt_sln->x[j] == 0 && curt_sln->cap_left + v1 >= v2) {
                    ...
                    if (delta > 0) {
                        ...
                        return new_sln; // Return the new solution after swapping (better solution)
                    } else { // Only consider worse moves if delta is negative
                        double probability = exp((double)-delta / temperature);
                        if ((double)rand() / RAND_MAX < probability) {
                            new_sln->x[item1] = 0; // Remove item1 from solution
                            new_sln->x[item2] = 1; // Add item2 to solution
                            new_sln->objective = curt_sln->objective + delta;  // Update objective value (worse objective)
                            new_sln->cap_left = new_sln->cap_left + curt_sln->prob->items[item1].v - curt_sln->prob->items[item2].v; // Update residual capacity
                            //printf("Worse move accepted: Temp = %.2f, Delta = %d, Probability = %.4f, Rand = %.4f, Current Objective = %d\n", temperature, delta, probability, (double)rand() / RAND_MAX, new_sln->objective);
                            return new_sln; // Worse solution accepted
                      }
                  }
                  // If delta == 0, we do NOT accept the swap in either SA or First Descent in this modified code.
              }
          }
        }
    }
    free_solution(new_sln); // Free if no move is made to avoid memory leak
    return NULL; // Return NULL if no move was made (neither better nor worse accepted)
}


// Modified Simulated Annealing function to use Lundy-Mees cooling schedule
struct solution* simulated_annealing(struct solution* init_sln, double beta) {
    struct solution *best_sln, *curt_sln, *nb_sln;
    curt_sln = copy_from(init_sln);
    best_sln = copy_from(init_sln);

    double temperature = 100; // Increased initial temperature
    // double cooling_rate = 0.95; // No longer used

    srand(time(NULL));
    clock_t start_time = clock();

    printf("SA Progress with Lundy-Mees Cooling (beta = %.4f):\n", beta); // Debugging header

    for (int k = 0; k < MAX_NUM_OF_ITERS; k++) {
        if ((double)(clock() - start_time) / CLOCKS_PER_SEC > MAX_TIME) {
            printf("Time limit exceeded, stopping SA.\n");
            break;
        }

        for (int move_count = 0; move_count < MOVES_PER_TEMP; move_count++) { // Inner loop for multiple moves
            nb_sln = simulated_annealing_move(curt_sln, temperature);

            if (nb_sln != NULL) {
                free_solution(curt_sln);
                curt_sln = copy_from(nb_sln);
                if (curt_sln->objective > best_sln->objective) {
                    free_solution(best_sln);
                    best_sln = copy_from(curt_sln); // Corrected line: best_sln = copy_from(curt_sln);
                    printf("Iter %d, Temp %.2f, Current Objective: %d, Best Objective: %d, Improvement!\n", k, temperature, curt_sln->objective, best_sln->objective); // Track improvement
                } else if (nb_sln->objective < curt_sln->objective){
                    printf("Iter %d, Temp %.2f, Current Objective: %d, Best Objective: %d, Worse accepted.\n", k, temperature, curt_sln->objective, best_sln->objective); // Track worse move
                }
                free_solution(nb_sln);
            } else {
                //printf("Iter %d, Temp %.2f, Current Objective: %d, Best Objective: %d, No move.\n", k, temperature, curt_sln->objective, best_sln->objective); // Track no move - optional, can be verbose
            }
        }

        // Lundy-Mees nonlinear cooling function
...
int main() {
    ...
    struct solution* sa_optimum_sln;
    double beta_lundy_mees = 0.01; // Example beta value, tune as needed
    sa_optimum_sln = simulated_annealing(copy_from(init_sln), beta_lundy_mees); // Run Simulated Annealing with Lundy-Mees cooling
    print_sol(sa_optimum_sln, "Simulated Annealing Solution with Lundy-Mees Cooling");

    // Free memory for all pointers
    free_problem(prob);
    free_solution(init_sln);
    free_solution(local_optimum_sln);
    free_solution(sa_optimum_sln);
    return 0;
}