...
struct solution* best_descent(struct solution* curt_sln) {
    int item1, item2;
    struct solution* best_sln = NULL; // To store the best solution found
    int best_delta = 0; // To store the best improvement in objective value
    ...
    for (int i = 0; i < NUM_OF_ITEMS; i++) { // Iterate over all items
        if (curt_sln->x[i] > 0) { // If item i is currently included in the solution
            ...
            for (int j = 0; j < NUM_OF_ITEMS; j++) { // Iterate over all items again for potential swaps
                ...
                // Check if item j is not the same item, not in the solution, and swapping is feasible
                if (i != j && curt_sln->x[j] == 0 && curt_sln->cap_left + v1 >= v2) {
                    ...
                    if (delta > best_delta) {
                        ...
                        if (best_sln != NULL) {
                            free_solution(best_sln); // Free the previous best solution
                        }
                        best_sln = copy_from(curt_sln); // Copy current solution
                        best_sln->x[item1] = 0; // Remove item1 from solution
                        best_sln->x[item2] = 1; // Add item2 to solution
                        best_sln->objective = curt_sln->objective + delta; // Update objective value
                        best_sln->cap_left = curt_sln->cap_left + v1 - v2; // Update residual capacity
                        best_delta = delta; // Update the best delta
                    }
                }
            }
        }
    ...
    return best_sln; // Return the best solution found (or NULL if no improvement)
}
...
struct solution* local_search(struct solution* init_sln) {
    ...
    for (int k = 0; k < MAX_NUM_OF_ITERS; k++) {
        nb_sln = best_descent(curt_sln); // Use best descent instead of first descent
        if (nb_sln != NULL) {
            free_solution(curt_sln); // Free the current solution
            curt_sln = copy_from(nb_sln); // Update current solution to the best solution found
            free_solution(nb_sln); // Free the best solution
        } else {
            break; // If no improvement is found, stop the search
        }
    }

    return curt_sln; // Return the local optimum solution
}
...
int main() {
    ...
    local_optimum_sln = local_search(init_sln); // local search with best descent
    print_sol(local_optimum_sln, "Best Descent Local Optima Solution");
