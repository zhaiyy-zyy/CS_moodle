...
def random_search_fit(items, capacity, fit_fun, iterations=15000):
    ...
    elif items_count > 200:
        # 物品数量在200-500之间，适当减少迭代次数
        actual_iterations = min(10000, iterations)
    elif items_count > 100:
        # 物品数量在100-200之间
        actual_iterations = min(15000, iterations)
    else:
        # 物品数量小于100，保持原有迭代次数
    ...
    for i in range(actual_iterations):
        ...
        if i > 100 and i % 100 == 0:
            last_100_iterations_improved = False
            for j in range(1, 101):
                if len(best_solution) < min_bins + j:
                    last_100_iterations_improved = True
                    break
            if not last_100_iterations_improved:
                break
